var Monitor = Monitor || (function() {
  ...
  var memoryTpl =
    '<div ag-if="scope.memoryEnable" ag-style="{width:scope.memoryWidth}" style="position:fixed;right:0;bottom:0;opacity:0.9;cursor:pointer;">' +
    ...
    '         <span ag-each="slot in scope.slots" ag-style="{\'height\': slot.height, \'margin-top\':30-slot.height, \'background-color\': slot.color, \'width\':memorySlotWidth}" style="float:right;"></span>' +
    '     </div>' +
    '  </div>' +
    '</div>';
  var frameRateTpl =
    '<div ag-style="{width:scope.rateWidth, right: scope.memoryEnable ? scope.memoryWidth : 0}" style="opacity:0.9;cursor:pointer;position:fixed;bottom:0px;">' +
    '   <div style="padding:0 0 3px 3px;text-align:left;background-color:#020;">' +
    '      <div style="color:#0f0;font-family:Helvetica,Arial,sans-serif;font-size:9px;font-weight:bold;line-height:15px">' +
    '         Repaint rate: ${scope.rate}/sec' +
    '      </div>' +
    '   </div>' +
    '</div>';
