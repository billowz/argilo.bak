...
import { onMatchCallback, onErrorCallback, Rule, discardMatch } from './Rule'
...
function mkMatch(
	args: IArguments | any[] | MatchRuleDescriptor,
	defaultPick?: boolean | number,
	defaultOnMatch?: onMatchCallback
): MatchRule {
	let name: string,
		pattern: number | string | any[],
		regexp: RegExp,
	...
	return regexp
		? regMatch(
				name,
				regexp,
				isBool(args[i]) || isInt(args[i]) ? pick : defaultPick,
				startCodes,
				capturable,
				onMatch,
				onErr
...
function rulesBuilder(rules: ((rule: Rule) => any[]) | any[]): (rule: Rule) => Rule[] {
	return function(_rule) {
		return mapArray(isFn(rules) ? (rules as ((rule: Rule) => any[]))(_rule) : rules, (r, i) => {
			if (!r) return SKIP
			let rule: Rule = r.$rule ? r : isArray(r) ? mkMatch(r) : mkMatch([r], false, discardMatch)
