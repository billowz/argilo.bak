...
if (!defPropSupport) {
  __defProp = function __defProp(obj, prop, desc) {
    devlevel.assert(desc.get || desc.set, 'not support getter/setter on defineProperty');
    obj[prop] = desc.value;
    return obj;
  };
}
...
function __extend(cls, mixins, offset, filter, data) {
  devlevel.assert.fn(cls, 'Invalid Class');
...
function createClass(overrides) {
  ...
  devlevel.assert(!constructor || isFn(constructor), 'Invalid Constructor');
  devlevel.assert(!superCls$$1 || isFn(superCls$$1), 'Invalid Super Class');
...
inherit(List, {
  ...
  each: function each$$1(cb, scope) {
    devlevel.assert(!this.scaning, 'Recursive calls are not allowed.');
    this.scaning = true;
    ...
    if (this.length) {
      ...
      while (node) {
        if (node[3] === this && (newtail = cb(node[0]))) {
          ...
          devlevel.assert(!newtail[3], 'Double add List, Clone Callback should return a new Object');
          newtail[3] = newlist;
...
function __insertAll(list, objs, prev) {
  ...
  devlevel.assert(!head[3], 'Double add List, Object have added in this List');
  head[3] = list;
  var __prev = head,
      tail = head,
      i = 1;
  ...
  for (; i < l; i++) {
    ...
    devlevel.assert(!tail[3], 'Double add List, Object have added in this List');
    tail[3] = list;
    tail[1] = __prev;
    __prev[2] = tail;
    __prev = tail;
...
function __initNode(list, obj) {
  ...
  if (node && node[0] === obj) {
    devlevel.assert(!node[3] || node[3] === list, 'Double add List, Object is still in other List');
  } else {
...
function __getNode(list, obj) {
  ...
  if (node && node[0] === obj) {
    devlevel.assert(node[3] === list, 'Object is not in this List');
    return node;
  }

  devlevel.assert(0, 'Object is not in List');
}
...
inherit(FnList, {
  add: function add(fn, scope, data) {
    devlevel.assert.fn(fn, 'fn is not a Function.');
  ...
  each: function each$$1(cb, scope) {
    devlevel.assert.fn(cb, 'callback is not a Function.');
    if (scope) cb = cb.bind(scope);
...
function parsePath(path, cache) {
  ...
  devlevel.assert.str(path, "Require path[Array | String]");
...
inherit(Rule, {
  $rule: true,
  error: function error(stream, err, capturable, srcErr) {
  ...
  success: function success(stream, data, result) {
    ...
    devlevel.assert(stream.curr == curr);
...
function match(name, pattern) {
  ...
  devlevel.error("Invalid Match Rule: " + pattern);
}
...
var ComplexRule = inherit(function ComplexRule(name, rules, option) {
  ...
  this.getRules = function getLazyRules() {
    ...
    devlevel.assert(len, "Require Rules"); // parse Expression
...
function parseComplexRules(curr, rules, dist) {
  for (var i = 0, l = rules.length; i < l; i++) {
    if (!rules[i]) devlevel.error("Empty Rules[" + i + "]: " + rules);
...
function parseRule(curr, rule, dist) {
  if (rule.$rule) {
    ...
    devlevel.error("Invalid Rule: " + rule);
  }
} // ================================================
// Complex Rules
// ================================================
...
devlevel.assert(policy, 'Observer is not supported.');
...
devlevel.info("Observer: enable policy[" + policy.name + "]: ", {
  proxyEnabled: proxyEnabled,
  proxyChangeable: proxyChangeable,
  sourceOwnProperty: sourceOwnProperty
}, 'default properties: ', defaultProps$1);
...
inherit(Observer, {
  ...
  observe: function observe(path, handler, scope) {
    devlevel.assert.fn(handler, 'require function handler.');
    path = parsePath(path);
    var l = path.length - 1;
    devlevel.assert(l >= 0, 'require non-empty path.');
    ...
    for (; i <= l; i++) {
      ...
      if (!desc) {
        ...
        if (currentObserver) {
          devlevel.assert(!currentObserver.isArray || ARRAY_EVENTS[attr] && i == l, function () {
  ...
  unobserve: function unobserve(path, handler, scope) {
    ...
    path = parsePath(path);
    var l = path.length;
    devlevel.assert(l, 'require non-empty path.');
  ...
  if (proxyChangeable) {
    extend(Observer, {
      ...
      onProxyChange: function onProxyChange(fn, scope) {
        devlevel.assert.fn(fn, 'require function handler.');
        var list = this.proxyListens;
...
var VNode = inherit(function VNode(comp) {
  devlevel.assert(comp.$comp, 'Require Compontent');
  this.comp = comp;
  ...
  devlevel.assert(parent.$virtComplexElement, 'parent should be Virtual Complex Element');
  ...
  devlevel.assert(target[PARENT], 'target is Root Node');
  devlevel.assert(target[VIRT_NODE], 'target should be Virtual Node');
...
inherit(TransformExpression, Expression, {
  ...
  checkRestore: function checkRestore() {
    ...
    devlevel.assert.by(function () {
      _this.reachFilter(function (filter$$1) {
        devlevel.assert.fn(filter$$1.restore, "Transform[" + name + "] not support restore");
      });
    });
  },
  ...
  parseFilter: function parseFilter(name) {
    ...
    devlevel.assert(transform, "Transform[" + name + "] is undefined");
    devlevel.assert(isFn(transform) || isFn(transform.transform), "Invalid Transform[" + name + "].transform handler on " + (localTransforms && localTransforms[name] ? 'Local' : 'Global') + " Transforms");
    devlevel.assert(!transform.restore || isFn(transform.transform), "Invalid Transform[" + name + "].restore handler on " + (localTransforms && localTransforms[name] ? 'Local' : 'Global') + " Transforms");
    return transform;
  }
});
...
var VElement = inherit(function VElement(comp, option) {
  ...
  if (directives) {
    ...
    devlevel.assert.by(l, function () {
      for (var i = 0, g, D; i < l; i++) {
        g = directives[i];
        D = g.Directive;
        devlevel.assert(_this[D[PROTOTYPE].$nodeKey], "Can not bind directive[" + g.dname + ": " + fnName(D) + "] on " + fnName(_this.constructor) + ", require " + D[PROTOTYPE].$nodeName);
      }
    });
...
var VComplexElement = inherit(function VComplexElement(comp, option$$1) {
  ...
  if (children) {
    ...
    for (var i = 0, N; i < l; i++) {
      N = children[i].VNode;
      devlevel.assert(N[PROTOTYPE][this.$childKey], "Child Node[" + i + ": " + children[i].ename + "] should be " + this.$childName);
    }
  }
  ...
  devlevel.assert(node[this.$childKey], "node should be " + this.$childName);
  devlevel.assert(this[CHILD_NODES].has(target), "target is not existing in this Node");
  node.remove();
...
var Compontent = inherit(function Compontent(scope, option$$1) {
  ...
  if (!isObj(scope)) {
    devlevel.assert(scope && scope.$comp, 'Require Parent Compontent');
    comp = scope;
    scope = comp.$scope;
  }
...
function registerDirective(name, directive) {
  ...
  devlevel.assert(!directives[name], "Double Register Directive[" + name + "]");
  ...
  devlevel.assert(directive[PROTOTYPE].$directive, "Invalid Directive[" + name + "], should extend Directive");
  directives[name] = directive;
  devlevel.info("Register Directive: " + fnName(directive) + " [" + name + "]");
  return directive;
}
function directive(name, value, params) {
  var Directive = directives[name];
  devlevel.assert(Directive, "Directive[" + name + "] is undefined");
  if (!params) params = {};
  params.value = value;
...
function registerVElement(name, VElement) {
  devlevel.assert(!VElements[name], "Double Register Virtual Element: " + fnName(VElement) + " <" + name + ">");
  devlevel.assert(VElement[PROTOTYPE][VIRT_ELEMENT], "Invalid Virtual Element: " + fnName(VElement) + " <" + name + ">");
  VElements[name] = VElement;
  devlevel.info("register Virtual Element: " + fnName(VElement) + " <" + name + ">");
  return VElement;
}
function registerVText(VText) {
  devlevel.assert(!Text, "Double Register Virtual Text");
  devlevel.assert(VText[PROTOTYPE][VIRT_TEXT], "Invalid Virtual Node: " + fnName(VText));
  Text = VText;
  devlevel.info("register Virtual Text: " + fnName(VText));
  return Text;
}
function registerVBindingText(VBindingText) {
  devlevel.assert(!BindingText, "Double Register Virtual Binding Text");
  devlevel.assert(VBindingText[PROTOTYPE][VIRT_BINDING_TEXT], "Invalid Virtual Binding Text: " + fnName(VBindingText));
  BindingText = VBindingText;
  devlevel.info("register Virtual Binding Text: " + fnName(VBindingText));
  return BindingText;
}
function registerVComment(VComment) {
  devlevel.assert(!Comment, "Double Register Virtual Comment ");
  devlevel.assert(VComment[PROTOTYPE][VIRT_COMMENT], "Invalid Virtual Comment: " + fnName(VComment));
  Comment = VComment;
  devlevel.info("register Virtual Comment : " + fnName(VComment));
  return VComment;
}
function elem(name, attrs, directives, children) {
  var VElement = VElements[name];
  if (children && !children.length) children = undefined;
  devlevel.assert(VElement, "Virtual Element[" + name + "] is undefined");
  devlevel.assert(!children || !children.length || VElement[PROTOTYPE][VIRT_COMPLEX_ELEMENT], "Virtual Element[" + name + "] have no children");
...
var EventMixin = {
  ...
  on: function on(type, fn, scope) {
    devlevel.assert.fn(fn, 'Invalid Event Listener');
...
inherit(EventExpression, Expression, {
  ...
  parseFilter: function parseFilter(name) {
    ...
    devlevel.assert(filter$$1, "EventFilter[" + name + "] is undefined");
    devlevel.assert(isFn(filter$$1), "Invalid EventFilter[" + name + "] on " + (localFilters && localFilters[name] ? 'Local' : 'Global') + " EventFilters");
    return filter$$1;
  }
});
  ...
  for (var i = 0, code, key, j, m; i < l; i++) {
    ...
    if (isStr(code)) {
      ...
      if (isNaN(code)) {
        // key code
        code = keyCodes[key];
        devlevel.assert(isInt(code) || isArray(code), "Invalid Key Code[" + key + "]");
  ...
  handler: function handler(e) {
    ...
    if (expr.filter(comp, args)) {
      ...
      if (isFn(ret)) {
        ...
        devlevel.assert(!expr.sample, "Invalid Event Handler: " + this.params.value);
      }
    }
  },
  bind: function bind() {
...
__register('input', {
  ...
  constructor: function constructor(node, params) {
    ...
    devlevel.assert(this.expr.simple, "Invalid Expression[" + params.value + "] on directive[input]");
