import { assert, error, info } from 'devlevel';
...
var getObserver = function getObserver(obj) {
...
if (proxyEnabled) {
  ...
  source = function source(obj) {
    var observer = obj && getObserver(obj);
  ...
  $eq = function $eq(o1, o2) {
    return eq$1(o1, o2) || (o1 && o2 && (o1 = getObserver(o1)) ? o1 === getObserver(o2) : false);
  };
  ...
  if (proxyChangeable) {
    ...
    onProxyChange = function onProxyChange(obj, fn, scope) {
      var observer = getObserver(obj);
      return observer && observer.onProxyChange(fn, scope);
    };

    unProxyChange = function unProxyChange(obj, fn, scope) {
      var observer = getObserver(obj);
...
function $eachObj(obj, callback, scope, own) {
  ...
  if (own === false) {
    for (key in obj) {
      if (!isDefaultProp(key) && callback(proxy(obj[key]), key, obj) === STOP$1) return key;
    }
  } else {
    for (key in obj) {
      if ($hasOwnProp(obj, key) && callback(proxy(obj[key]), key, obj) === STOP$1) return key;
    }
  }
}
...
function __assignUserFilter$1(prop, dist, source$$1, cb) {
  return $hasOwnProp(source$$1, prop) && cb(prop, dist, source$$1);
}

function __assignIfFilter$1(prop, dist, source$$1) {
  return $hasOwnProp(source$$1, prop) && !$hasOwnProp(dist, prop);
}

function __assignFilter$1(prop, dist, source$$1) {
  return $hasOwnProp(source$$1, prop);
}
...
var Compontent = inherit(function Compontent(scope, option$$1) {
  ...
  onProxyChange: function onProxyChange$$1(fn, scope) {
    this.$observer.onProxyChange(fn, scope);
    return this;
  },
  unProxyChange: function unProxyChange$$1(fn, scope) {
...
export { __global as global, create, assign, assignIf, assignBy, extend, extendIf, extendBy, inherit, superCls, subclassOf, createClass, each, eachArray, eachStr, eachObj, reach, reachArray, reachStr, eachChain, STOP$1 as STOP, map, mapArray, mapStr, mapObj, SKIP, filter, filterArray, filterStr, filterObj, find, findArray, findStr, findObj, rfind, rfindStr, indexOf, indexOfArray, indexOfObj, lastIndexOf, lastIndexOfArray, obj2array, keys, values, eq$1 as eq, isNull, isUndef, isNil, isFn, is, isObj, isBool, isNum, isStr, isDate, isReg, isArray, isInt, isTypedArray, isArrayLike$1 as isArrayLike, isInstOf, isPrimitive, isBlank, applyScope, applyNoScope$1 as applyNoScope, applyScopeN, applyNoScopeN, apply, applyN, createFn, fnName, trim$1 as trim, upperFirst, strval, charCode, genCharCodes, reEscape, reUnion, regStickySupport, regUnicodeSupport, array2obj, makeMap, makeArray, cached, hasOwnProp, getOwnProp, prototypeOfSupport, prototypeOf, setPrototypeOf, defPropSupport, defProp, defPropValue, ASTBuilder as Parser, List, FnList, nextTick, clearTick, escapeString, pad, plural, singular, thousandSeparate, format, parsePath, formatPath, get, set, defaultProps$1 as defaultProps, isDefaultProp, isProxyEnabled, $set, observer$1 as observer, observe, unobserve, isObserved, getObserver, $hasOwnProp, source, proxy, $eq, onProxyChange, unProxyChange, $getOwnProp, $eachArray, $reachArray, $each, $reach, $eachObj, $mapArray, $mapObj, $map, $filterArray, $filterObj, $filter, $findArray, $findObj, $find, $rfindArray, $rfind, $indexOfArray, $indexOfObj, $lastIndexOfArray, $indexOf, $lastIndexOf, $assign, $assignIf, $assignBy, $obj2array, $keys, $values, tags, registerVElement, registerVText, registerVBindingText, registerVComment, elem, text, comment, VNode, VText, VBindingText, VElement, VComplexElement, Compontent, VComment, TransformExpression, transforms, directives, Directive, registerDirective, directive, Element as HTMLElement, ComplexElement as HTMLComplexElement, Compontent$1 as HTMLCompontent, Comment$1 as HTMLComment, HTMLText, HTMLBindingText, ExpressionDirective, EventDirective, DomDirective, eventFilters, EventKeyword, EventExpression, keyCodes, keywords, IDENTITY_OPT_GET, IDENTITY_OPT_SET, IDENTITY_OPT_CALL, Expression, ElemContent as NodeParser };
//# sourceMappingURL=argilo.es.js.map
